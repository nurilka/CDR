...
static ListItr	Find_Link (const  HashMap* _map, size_t _ind, const void* _key)
{
	...
	end 	= ListItr_End 	(_map->m_buckets[_ind]);
	
	while ( !ListItr_Equals (current, end) )
	{
		data = ListItr_Get	(current);
		
		if ( _map->m_isEqual(data->m_key, _key) )
		{
			return current;
		}
...
Map_Result HashMap_Rehash (HashMap *_map, size_t _newCapacity)
{
	...
	for ( i = 0; i < currentCap; ++i )
	{
		...
		while ( !ListItr_Equals (itr,end) )	
		{
			...
			if ( LIST_SUCCESS != err )
			{
				...
				for (j = 0; j <= i; j++) 
				{
					if ( tmpBuckets[j] ) 
					{
						List_Destroy( &tmpBuckets[j], NULL);
					}
				}
				free (tmpBuckets);
				return MAP_ALLOCATION_ERROR;
		...
		if ( ListItr_Equals(ListItr_Begin(_map->m_buckets[index]), ListItr_End(_map->m_buckets[index])) )
		{
...
size_t HashMap_ForEach (const HashMap* _map, KeyValueActionFunction _action, void* _context)
{
	...
	for ( i = 0; i < _map->m_capacity; ++i )
	{
		...
		while ( !ListItr_Equals (itr, end) )	
		{
			next = ListItr_Next (itr);
			data = (Pair*)ListItr_Get (itr);
			if ( _action( data->m_key, (void*)data->m_value, _context) )
			{	
				++counter;
			}
			else
